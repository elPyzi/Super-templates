import * as React from 'react';
import { ReactNode, useId } from 'react';

import { Field, FieldDescription, FieldError, FieldLabel } from '@shared-ui';
import { cn } from '@shared-libs/shadcn/utils';

type TextareaClassNames = {
  textareaWrapperClassName: string;
  labelClassName: string;
  errorClassName: string;
  descriptionClassName: string;
  textareaClassName: string;
};

type TextareaProps = React.ComponentProps<'textarea'> & {
  label?: ReactNode;
  error?: ReactNode;
  description?: ReactNode;
  classes?: TextareaClassNames;
  required?: boolean;
  colon?: boolean;
};

function Textarea({
  label,
  error,
  description,
  classes,
  required = false,
  colon = false,
  ...props
}: TextareaProps) {
  const id = useId();
  const isInvalid = !!error;

  return (
    <Field className={classes?.textareaWrapperClassName}>
      {label && (
        <FieldLabel htmlFor={id} className={classes?.labelClassName}>
          {`${label}${colon ? ':' : ''}`}
          {required && <span className="text-destructive">*</span>}
        </FieldLabel>
      )}
      {description && (
        <FieldDescription className={classes?.descriptionClassName}>
          {description}
        </FieldDescription>
      )}
      <textarea
        id={id}
        data-slot="textarea"
        aria-invalid={isInvalid}
        className={cn(
          'border-input placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:bg-input/30 flex field-sizing-content min-h-16 w-full rounded-md border bg-transparent px-3 py-2 text-base shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 md:text-sm',
          classes?.textareaClassName,
        )}
        {...props}
      />
      {error && (
        <FieldError id={id} className={classes?.errorClassName}>
          {error}
        </FieldError>
      )}
    </Field>
  );
}

export { Textarea };
